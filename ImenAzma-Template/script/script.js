$(function(){
	...
	var newsTrain = $('div.projects-list div.newsMainCase div.newsTrain'),
		...
		isize = lists.size(),
		num2case = 2, // 7 /// num of li(es) in newsMainCase that are visible
		itop = 0;
	...
	(startSliding = function(){
		if(intervalName) return;
		intervalName = setInterval(next_slide,4000);
	})();
	...
	$('header section.header div.login a').click(function(){
		$('header section.header div.loginpanel').animate({
				top:'5px'
			},300);
	});

	$('header section.header div.loginpanel a.close').click(function(){
		$('header section.header div.loginpanel').animate({
				top:'-500px'
			},200);
	});
